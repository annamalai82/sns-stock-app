...
import { STAFF, SECTIONS, ORDERING_SCHEDULE, DEFAULT_THRESHOLDS, dateKey } from './config';
...
export function checkLowStock(items, thresholds) {
  const th = thresholds || DEFAULT_THRESHOLDS;
  return items
    .filter(i => i.quantity <= (th[i.name] || th.default || 2))
    .map(i => ({
      ...i,
      threshold: th[i.name] || th.default || 2,
      severity: i.quantity === 0 ? 'critical' : 'warning',
    }));
}

export function generateResponse(input, stockData, user, logs, thresholds) {
  ...
  if (/low\s*stock|shortage|running\s*(low|out)|what.*need/i.test(lo) && !/schedule/i.test(lo)) {
    ...
    for (const [loc, secs] of Object.entries(stockData))
      for (const [sec, items] of Object.entries(secs))
        checkLowStock(items, th).forEach(a => all.push({ ...a, location: loc, section: sec }));
  ...
  if (/what('?s| is).*stock|show.*stock|check.*stock|status/i.test(lo) || /stock.*\?/.test(lo)) {
    ...
    if (sec && stockData[loc]?.[sec]) {
      ...
      const al = checkLowStock(items, th);
      let m = `📊 *${sec} — ${loc}*\n\n`;
      items.forEach(i => {
  ...
  if (parsed.length >= 2) {
    ...
    const al = checkLowStock(parsed, th);
